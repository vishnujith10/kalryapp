...
export default function CardioPlayerScreen({ route, navigation }) {
  ...
  useEffect(() => {
    ...
    return () => {
      try {
        soundObj && soundObj.unloadAsync && soundObj.unloadAsync();
      } catch {}
    };
  ...
  const nextExercise =
    !isRest && exerciseIdx < exercises.length - 1
      ? exercises[exerciseIdx + 1]
      : isRest && exerciseIdx < exercises.length
  ...
  const progress =
    timer /
    (isRest ? restBetweenRounds : parseInt(currentExercise?.duration) || 1);
    ...
    <View style={styles.background}>
      ...
      <View style={{ paddingTop: 32, paddingHorizontal: 24 }}>
        ...
        <Text style={styles.progressText}>
          Round {round} of {totalRounds} 7b7 Exercise {exerciseIdx + 1} of{" "}
          {totalExercises}
        </Text>
        <View style={styles.progressBarBg}>
          <View
            style={[
              ...
              {
                width: `${
                  ((exerciseIdx + (round - 1) * totalExercises) /
                    (totalExercises * totalRounds)) *
                  100
                }%`,
              },
            ]}
      ...
      {currentExercise?.img && !isRest && (
        <Image
          source={{ uri: currentExercise.img }}
      ...
      {nextExercise && !isRest && (
        <View style={styles.nextExerciseCard}>
          {nextExercise.img ? (
            <Image
              source={{ uri: nextExercise.img }}
              style={styles.nextExerciseImg}
            />
          ) : (
            <View
              style={[
                styles.nextExerciseImg,
                {
          ...
          <View style={{ flex: 1, marginLeft: 10 }}>
            ...
            <Text style={styles.nextExerciseName}>
              {nextExercise.name} - {nextExercise.duration}s
            </Text>
          </View>
        </View>
      )}
